...
namespace Network
{
	HttpClientConnection::HttpClientConnection(boost::asio::io_service& io_service,
	...
	void HttpClientConnection::running()
	{
		checkTimeout();
	}
	void HttpClientConnection::open(HttpRequestPtr spRequest)
	{
	...
	void HttpClientConnection::handleResolve(const boost::system::error_code& err,
		boost::asio::ip::tcp::resolver::iterator endpoint_iterator)
	{
		...
		boost::asio::async_connect(mkSocket, endpoint_iterator,
			boost::bind(&HttpConnection::handleConnect, shared_from_this(),
			boost::asio::placeholders::error));
	}

	void HttpClientConnection::handleConnect(const boost::system::error_code& err)
	{
		if (err)
		{
			setStatus(Status::error_occur);
		...
		boost::asio::async_write(mkSocket, mstrRequest,
	...
	void HttpClientConnection::handleWriteRequest(const boost::system::error_code& err,
		std::size_t bytes_transferred)
	{
		...
		boost::asio::async_read_until(mkSocket, mstrResponse, "\r\n\r\n",
	...
	void HttpClientConnection::handleReadStatusLineAndHeaders(const boost::system::error_code& err,
		std::size_t bytes_transferred)
	{
		...
		boost::asio::async_read(mkSocket, mstrResponse,
	...
	void HttpClientConnection::handleReadMessageBody(const boost::system::error_code& err,
		std::size_t bytes_transferred)
	{
		if (bytes_transferred <= 0)
		{
	...
	void HttpClientConnection::checkTimeout()
	{
